...
from. Ticket 18 replaced the authored demonstration with the analysis's own output:
`lib/landing/sample-analysis.ts` is generated by running `analyze()` over
`tests/fixtures/adhesion-contract.txt` and is regenerated with `npm run sample`, and
`tests/landing-sample.test.ts` fails when the committed file and the analysis disagree.
The page names the fixture and says the run used the test suite's stub gateway rather
than the live model. No layout or type decision from the finish review changed.
