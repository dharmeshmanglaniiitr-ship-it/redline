...
Covers all nine v1 capabilities in one document. Not published to an issue tracker —
none is configured for this repo yet.
...
Version diffing between contract drafts. Not among the nine capabilities; would need
...
`ready-for-agent` label. Note that at nine capabilities it is an implementation
reference rather than a single unit of agent-ready work, and will likely need slicing
